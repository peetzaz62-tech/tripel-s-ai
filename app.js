...
function extBackgroundParagraph(bg){
  ...
  if(bg === 'low') return `Background & Horizon: the building stands on a flat suburban street, with its neighbours to either side of it along that same street — contemporary one and two storey houses set well apart, each a clean modern volume with a flat or shallow roof, smooth painted and timber-clad walls, large glazing and a carport, with young street trees and low planting between them. They stand on ground as level as the road, each seen from the side at its own height, so no house ever appears above the main building's roofline. Directly behind the main building there is only open sky and the tops of distant trees. Only distance makes each house smaller, softer and lower in contrast, and each reads as a house built in the last few years.`;
